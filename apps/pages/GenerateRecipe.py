...
if st.session_state.get("last_followup_was_update") and "followup_feedback_given" not in st.session_state:
    ...
    with col2:
        if st.button("👎 Not quite", use_container_width=True, key="followup_thumbs_down"):
            ...
            st.rerun()
elif st.session_state.get("last_followup_was_update") and st.session_state.get("followup_feedback_given"):
...
if user_followup:
    ...
    # If a confirmation is pending and the user typed a plain yes/no instead
    # of clicking the button, handle it directly here.
    handled_as_confirmation = False
    if st.session_state.get("pending_confirmation"):
        normalized = user_followup.strip().lower()
        if normalized in ("yes", "y", "yeah", "yep", "sure", "please do", "go ahead", "ok", "okay"):
            st.session_state["pending_confirmation"] = False
            confirm_prompt = (
                "Yes, please go ahead and update the recipe based on the suggestion you just made. "
                "Format ingredients as plain names only — no quantities, no units, no preparation verbs. "
            ...
            # User typed something other than a plain yes/no — clear the
            # pending flag so their new message goes through normal
            # classification below instead of being silently dropped.
            st.session_state["pending_confirmation"] = False

    if not handled_as_confirmation:
